...
type MessageModalProps = {
  ...
  type: MessageType;
  onClose: () => void;
  isExiting: boolean; // 非表示アニメーション用
};

// モーダルコンポーネント
const MessageModal: React.FC<MessageModalProps> = ({ message, type, onClose, isExiting }) => {
  const typeStyles = {
...
export const useMessageModal = () => {
  ...
  const [isExiting, setIsExiting] = useState(false); // 非表示アニメーション用
  ...
  const showModal = (text: string, messageType: MessageType, time: number = 4000) => {
    ...
    if (time > 0) {
      setTimeout(() => {
        setIsExiting(true); // 非表示アニメーション開始
      }, time);
    }
  };
  ...
  const Modal = () => {
    ...
    return <MessageModal message={message} type={type} onClose={() => setIsVisible(false)} isExiting={isExiting} />;
  };
